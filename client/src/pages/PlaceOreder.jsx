...
const PlaceOreder = () => {
  const {navigate,token,cartItmes,setCartItems,getCartAmount,delivery_fee,products} = useContext(ShopContext)
    ...
    try{
         ...
         switch(method){
          case "cod":
            ...
            case "stripe" :
              const resStripe = await axios.post('http://localhost:5000/api/order/stripe',orderData,{headers:{token}})
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        <div className='text-xl sm:text-2xl my-3'>
          <Title text1={'DELIVETY'} text2={'INFORMATION'}/>
        </div>
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='w-full text-end mt-8'>
            <button type='submit'  className='bg-black text-white cursor-pointer text-sm px-16 py-3 '>PLACE ORDER</button>
          </div>
        </div>
      </div>
    </form>
